"""
src/llm/evaluate_golden.py

Generates predictions for all 200 golden evaluation examples using the RAG + Ollama
SupportAgent (Step 10).

Outputs:
  results/rag_ollama_golden_predictions.jsonl
...
DEFAULT_OUTPUT_PATH = r"c:\CustomerSupport\results\rag_ollama_golden_predictions.jsonl"
DEFAULT_MODEL = "llama3.2:3b"
DEFAULT_TOP_K = 3


def generate_rag_golden_predictions(
    ...
    print(f"[evaluate_golden] Loaded {n_records} golden records from {golden_path}")
    if n_records != 200:
        print(f"WARNING: Expected 200 golden records, found {n_records}.")

    # 2. Initialize RAG Agent
    print(f"[evaluate_golden] Initializing SemanticRetriever and OllamaClient (model: {model_name}, top_k: {top_k})...")
    ...
    print(f"[evaluate_golden] Starting prediction loop over {n_records} examples...")
    predictions = []
    seen_ids = set()
    ...
    print(f"\n[evaluate_golden] Successfully saved {len(predictions)} predictions to: {output_path}")
    print(f"[evaluate_golden] Total execution time: {total_time:.2f}s ({total_time/n_records:.2f}s per example)")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate RAG + Ollama predictions for golden evaluation dataset.")
    parser.add_argument("--golden", default=DEFAULT_GOLDEN_PATH, help="Path to golden_dataset.jsonl")
    parser.add_argument("--output", default=DEFAULT_OUTPUT_PATH, help="Path to write rag_ollama_golden_predictions.jsonl")
...
if __name__ == "__main__":
    args = parse_args()
    generate_rag_golden_predictions(
